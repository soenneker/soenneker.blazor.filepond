...
{
    private static readonly Dictionary<string, (string uri, string integrity)> _styleCdnLinks = new()
    {
        {
    ...
    private static readonly Dictionary<string, (string uri, string integrity)> _scriptCdnLinks = new()
    {
        {
            nameof(FilePondPluginType.ImagePreview),
        ...
        string localBasePath = "_content/Soenneker.Blazor.FilePond/css/")
    {
        ...
        if (_styleCdnLinks.TryGetValue(type.Name, out (string uri, string integrity) cdnData))
        {
            if (useCdn)
                return cdnData;

            ReadOnlySpan<char> fileName = cdnData.uri.AsSpan(cdnData.uri.LastIndexOf('/') + 1);
            return (string.Concat(localBasePath.AsSpan(), fileName), null);
        }
        ...
        string localBasePath = "_content/Soenneker.Blazor.FilePond/js/")
    {
        ...
        if (_scriptCdnLinks.TryGetValue(type.Name, out (string uri, string integrity) cdnData))
        {
            if (useCdn)
                return cdnData;
